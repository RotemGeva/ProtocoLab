...
class Requirements:
    def __init__(self, parameters):
        ...
        try:
            ...
            logging.error(f'Make Requirement Error: {err}')
            sys.exit(1)
    ...
    def prepare_protocol_extractor_for_make_req(self):
        ...
        try:
            temp_folder_list = os.listdir('\\\\192.100.101.7\\Public\\Testing\\Tools\\ProtocolExtractor\\bin')
            shutil.copytree(
                f'\\\\192.100.101.7\\Public\\Testing\\Tools\\ProtocolExtractor\\bin\\{temp_folder_list[len(temp_folder_list) - 1]}',
                f'Data\\ProtocolExtractor')
        except Exception as err:
    ...
    def activate_protocol_extractor(self):
        ...
        logging.info(
            f'Running Protocol Extractor tool, command: {os.getcwd()}\\Data\\ProtocolExtractor\\ProtocolExtractor.exe -f {os.getcwd()}\\Data\\TempTarExtract -m {os.getcwd()}\\Data\\ProtocolExtractor\\fields_map.ini')
        try:
            subprocess.run(
        ...
        for file in os.listdir(f'{os.getcwd()}\\Data\\ProtocolExtractor'):
            if file.startswith('protocols_'):
                ...
                try:
                    ...
                    logging.info(
                        f'{file} moved from {os.getcwd()}\\Data\\ProtocolExtractor\\{file} to {os.getcwd()}\\Data'
                        f'\\Requirements')
                except Exception as err:
    ...
    def excel_rearrangement(self, parameters):
        logging.info(f'Start working on {self.mr}_Requirements.xlsx')
        logging.info(f'Open {self.mr}_Requirements.xlsx using Python')
        try:
        ...
        try:
            ...
            for sheet in requirement_file.sheetnames:
                logging.info(f'Verifying sheet: {sheet}')
        ...
        logging.info(f'Open {self.mr}_Requirements.xlsx for macros')
        try:
        ...
        try:
            ...
            logging.info(f'{self.mr}_Requirements.xlsx Saved')
        except Exception as err:
